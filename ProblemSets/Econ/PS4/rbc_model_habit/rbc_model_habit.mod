...
model;
	...
	(c-bb1*c(-1))^ (-1 / eeis)-bbeta*bb1*(c(+1)-bb1*c)^ (-1 / eeis)	= bbeta * ((c(+1)-bb1*c)^ (-1 / eeis)) * (1 - ddelta + r(+1));

	// 2. First order equation for labor-leisure
	w * ((c-bb1*c(-1))^ (-1 / eeis)-bbeta*bb1*(c(+1)-bb1*c)^ (-1 / eeis)		= 	cchi * (l ^ (1 / eeta));
...
steady_state_model;
	...
	z		= 0;
	w		= (1 - aalpha) * y / l;
	cchi		= w * (((1-bb1)*c) ^ (-1 / eeis)-bbeta*bb1*((1-bb1)*c)^(-1 / eeis)) / (l ^ (1 / eeta));
